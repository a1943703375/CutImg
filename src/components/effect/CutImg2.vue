...
let useCutImg : CutImg = new CutImg();
...
/* const { isSelected } = useCutImg()
                    .setVnode(props.tarImg, props.preImg, props.cutFrame)
                    .setProps(props.cutBoxW, props.whRatio, props.preW)
                    .getSelectStatus(); */


const isSelected = useCutImg.setId(props.tarImg, props.preImg, props.cutFrame).setProp(props.cutBoxW, props.whRatio, props.preW).getSelectStatus();
...
<template>
    <div class="container-cut">
        <!-- 未选择图片 -->
        <div class="no-select" v-show="!isSelected" @click="reset()">
            <svg class="upload-icon" viewBox="0 0 1029 1024" xmlns="http://www.w3.org/2000/svg" width="36" height="36" :style="themeStore.$state.isLight ? {fill : 'rgb(140,140,140)'} : {fill : 'rgb(110,110,110)'}">
            ...
            <div class="no-select-mask" :style="themeStore.$state.isLight ? {background : 'rgb(200,200,200)'} : {background : 'rgb(49, 53, 60)'}"></div>
            <span :style="themeStore.$state.isLight ? {color : 'rgb(50,50,50)'} : {color : 'rgb(150,150,150)'}">选择本地图片</span>
        </div>
 <!-- 已选择图片 -->
        <div v-show="isSelected">
            ...
            <div class="selected-upload" @click="reset()">
                <svg class="icon" viewBox="0 0 1024 1024" width="15" height="15" :style="themeStore.$state.isLight ? {fill : '#8a8a8a'} : {fill : 'rgb(190,190,190)'}">
                ...
                <span :style="themeStore.$state.isLight ? {color : 'rgb(50,50,50)'} : {color : 'rgb(190,190,190)'}">选择文件</span>
            </div>
        </div>
        <!-- 预览 -->
        <div class="preview">
            <!-- 图片 -->
            <div class="p-avatar">
                <div class="a-img-mask" :class="themeStore.$state.isLight ? 'a-img-mask-light' : 'a-img-mask-dark'" :style="isAvatar ? {borderRadius : '50%', width : preW + 'px', height : preW + 'px'} : {borderRadius : '5px', width : preW + 'px', height : preW / whRatio + 'px'}">
                    <img :id="preImg" src="@/assets/222.jpg" alt=" "/>
                </div>
                <span v-if="!isAvatar" :style="themeStore.$state.isLight ? {color : 'rgb(50,50,50)'} : {color : 'rgb(190,190,190)'}">预览图片</span>
                <span v-if="isAvatar" :style="themeStore.$state.isLight ? {color : 'rgb(50,50,50)'} : {color : 'rgb(190,190,190)'}">{{ !isSelected ? '当前头像' : '预览头像' }}</span>
                <div v-if="isSelected" class="submit" :style="themeStore.$state.isLight ? {background : 'rgb(56, 155, 243)'} : {background : 'rgb(47, 110, 165)'}">
                    <span :style="themeStore.$state.isLight ? {color : 'white'} : {color : 'rgb(200,200,200)'}">确定</span>
                </div>
            </div>
        </div>
    </div>
